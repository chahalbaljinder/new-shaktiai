...
export default function WishesVault() {
  ...
  const handleShareWish = async () => {
    ...
    // Validate email format for email sharing
    if (shareData.method === 'email') {
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
      if (!emailRegex.test(shareData.recipient)) {
        toast.error('Please enter a valid email address')
        return
      }
    }

    // Validate phone number for WhatsApp sharing
    if (shareData.method === 'whatsapp') {
      const phoneRegex = /^[\d\s\-\+\(\)]+$/
      if (!phoneRegex.test(shareData.recipient)) {
